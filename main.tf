...
# Explanation: EC2 SG is armageddon’s bodyguard—only let in what you mean to.
...
resource "aws_ssm_parameter" "armageddon_db_port_param" {
  name  = "/lab/db/port"
  type  = "String"
  value = tostring(aws_db_instance.armageddon_rds01.port)
...
resource "aws_secretsmanager_secret_version" "armageddon_db_secret_version01" {
  ...
  secret_string = jsonencode({
    username = var.db_username
    password = var.db_password
    host     = aws_db_instance.armageddon_rds01.address
    port     = aws_db_instance.armageddon_rds01.port
    dbname   = var.db_name
  })
}
...
# resource "aws_vpc_endpoint" "armageddon_vpce_ssm" { ... }
